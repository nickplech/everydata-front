...
function (_Component) {
  ...
  function ReviewMessage() {
    ...
    _defineProperty(_assertThisInitialized(_this), "saveTimeMin", function (e) {
      ...
      var _minutes = function minutes(value) {
        if (value === '15') {
          _minutes = '15';
        } else if (value === '30') {
          _minutes = '30';
      ...
      _this.state.time.includes('15');

      var built = _this.state.time.replace('15', "".concat(value));

      _this.setState({
        time: built
      });

      _this.state.time.includes('30');

      var built2 = _this.state.time.replace('30', "".concat(value));

      _this.setState({
        time: built2
      });

      _this.state.time.includes('45');

      var built3 = _this.state.time.replace('45', "".concat(value));

      _this.setState({
        time: built3
      });

      var built4 = _this.state.time.replace('00', "".concat(value));
...
//# sourceMappingURL=client.js.029dd9cf0c25d77f404d.hot-update.js.map